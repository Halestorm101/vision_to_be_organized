	...
	class BuilderAdminBar {
		...
		function add_admin_bar_menu_entry( &$wp_admin_bar ) {
			...
			if ( ! empty( $this->_layout_id ) ) {
				$wp_admin_bar->add_menu( array( 'parent' => 'builder', 'id' => 'builder_edit_layout', 'title' => sprintf( __( 'Edit Layout (%s)', 'it-l10n-Builder-Parker' ), $this->_layout_description ), 'href' => admin_url( 'admin.php?page=layout-editor&editor_tab=layouts&layout=' . $this->_layout_id ) ) );
				
				if ( 'on' != get_user_setting( 'widgets_access' ) )
					$wp_admin_bar->add_menu( array( 'parent' => 'builder', 'id' => 'builder_edit_widgets', 'title' => __( 'Manage Widgets for this Layout', 'it-l10n-Builder-Parker' ), 'href' => admin_url( 'widgets.php?builder_layout_id=' . $this->_layout_id ) ) );
			}
			
			$wp_admin_bar->add_menu( array( 'parent' => 'builder', 'id' => 'builder_edit_settings', 'title' => __( 'Modify Builder Settings', 'it-l10n-Builder-Parker' ), 'href' => admin_url( 'admin.php?page=theme-settings' ) ) );
