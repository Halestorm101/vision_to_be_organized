...
if ( ! class_exists( 'BuilderLayoutEditor' ) ) {
	class BuilderLayoutEditor extends ITCoreClass {
		...
		function __construct() {
			...
			$this->_page_title = _x( 'Manage Layouts and Views', 'page title', 'it-l10n-Builder-Parker' );
			$this->_menu_title = _x( 'Layouts & Views', 'menu title', 'it-l10n-Builder-Parker' );
		...
		function init() {
			...
			$this->_tabs = array(
				'layouts' => __( 'Layouts', 'it-l10n-Builder-Parker' ),
				'views'   => __( 'Views', 'it-l10n-Builder-Parker' ),
			);
...
?>
	<p><?php _e( 'Layouts are at the heart of how Builder gives you more control over your site. The goal is to allow you to create a site structure that meet your needs and the needs of your site rather than forcing you to structure your site to fit the mold of a specific theme.', 'it-l10n-Builder-Parker' ); ?></p>
	<p><?php _e( 'This editor allows you to create and modify Layouts for your site. There are a number of options, but most users will only use the following three regularly: Name, Width, and the Design section.', 'it-l10n-Builder-Parker' ); ?></p>
	<p><?php _e( 'The Name is just an easy way to keep track of a specific Layout and is also used as the base of the name for a Layout\'s Widget areas.', 'it-l10n-Builder-Parker' ); ?></p>
	<p><?php _e( 'The Width sets how wide the layout is. Since this is a Layout option, you can have Layouts of various widths used for different parts of your site.', 'it-l10n-Builder-Parker' ); ?></p>
	<p><?php _e( 'Extensions can modify how a layout functions and what the layout looks like. They are like mini-themes in that they can have style.css and functions.php files.', 'it-l10n-Builder-Parker' ); ?></p>
	<p><?php printf ( __( 'Using the Hide Widget Areas options allow for showing or hiding a Layout\'s widget areas from WordPress\' <a href="%s">Widgets editor</a>. Since a Builder site can have a very large number of widget areas, hiding select Layout\'s widget areas can make managing widgets much easier. This option can also be toggled from the main Layout listing by hovering over a Layout\'s row and selecting either "Show widget areas" or "Hide widget areas".', 'it-l10n-Builder-Parker' ), admin_url( 'widgets.php' ) ); ?></p>
	<p><?php _e( 'The Design area of this editor is where you assemble your layout using Modules. Each Module represents a different part of a layout. The Navigation Module adds a navigation menu. The Content Module holds the main content of that view, such as a listing of posts for the home page or page content if you are viewing a specific page. Clicking the "Add Module" link in the Design area shows the available modules and describes what they add to your Layout. By assembling a mix of desired Modules, you can quickly build a Layout for your site.', 'it-l10n-Builder-Parker' ); ?></p>
<?php
...
?>
	<p><?php _e( 'Layouts are at the heart of how Builder gives you more control over your site. The goal is to allow you to create a site structure that meet your needs and the needs of your site rather than forcing you to structure your site to fit the mold of a specific theme.', 'it-l10n-Builder-Parker' ); ?></p>
	<p><?php _e( 'For details about what a Layout is and how it is put together, click the "Create Layout" button below to create a new Layout from scratch or click the "Edit" link for a specific Layout and view the "Help" area from within the Layout editor.', 'it-l10n-Builder-Parker' ); ?></p>
	<p><?php _e( 'To create a new Layout, click one the "Create Layout" buttons below. As you hover over a Layout\'s row, additional options become visible. These additional options allow you to Edit, Duplicate, or Delete a Layout. They also offer options to set a Layout as the Default Layout and to show or hide widget areas.', 'it-l10n-Builder-Parker' ); ?></p>
...
<?php
		...
		function _hide_widget_areas() {
			...
			if ( ! empty( $successful ) ) {
				if ( 1 == count( $successful ) )
					ITUtility::show_status_message( sprintf( __( 'Widget areas for %s are now hidden from the <a href="widgets.php">Widgets editor</a>.', 'it-l10n-Builder-Parker' ), $successful[0] ) );
				else
					ITUtility::show_status_message( __( 'Widget areas for the requested Layouts are now hidden from the <a href="widgets.php">Widgets editor</a>.', 'it-l10n-Builder-Parker' ) );
				
				$this->_save();
			}
			else if ( ! $bulk )
				ITUtility::show_error_message( __( 'Unable to find the requested layout.', 'it-l10n-Builder-Parker' ) );
		...
		function _show_widget_areas() {
			...
			if ( ! empty( $successful ) ) {
				if ( 1 == count( $successful ) )
					ITUtility::show_status_message( sprintf( __( 'Widget areas for %s now appear in the <a href="widgets.php">Widgets editor</a>.', 'it-l10n-Builder-Parker' ), $successful[0] ) );
				else
					ITUtility::show_status_message( __( 'Widget areas for the requested Layouts now appear in the <a href="widgets.php">Widgets editor</a>.', 'it-l10n-Builder-Parker' ) );
		...
		function _reset_data() {
			...
			ITUtility::show_status_message( __( 'Data reset', 'it-l10n-Builder-Parker' ) );
		...
		function _modify_module_settings( $data = false ) {
			...
			if ( method_exists( $this->_modules[$module], 'edit' ) ) {
				...
				echo '<h1>' . sprintf( __( 'Modify %s Module Settings', 'it-l10n-Builder-Parker' ), $this->_modules[$module]->_name ) . "</h1>";
...
<?php
		...
		function _delete_layout() {
			...
			foreach ( (array) $layouts as $layout ) {
				...
				if ( ( $this->_options['layouts'][$layout]['total_num_views'] > 0 ) || ( $layout === $this->_options['default'] ) ) {
					if ( $bulk || empty( $_POST['replacement_layout'] ) ) {
						if ( $bulk ) {
							if ( ( $this->_options['layouts'][$layout]['total_num_views'] > 0 ) && ( $layout === $this->_options['default'] ) )
								ITUtility::show_error_message( sprintf( __( 'The %s Layout is both the Default Layout and has Views relying on it. It will need to be manually deleted.', 'it-l10n-Builder-Parker' ), $description ) );
							else if ( $this->_options['layouts'][$layout]['total_num_views'] > 0 )
								ITUtility::show_error_message( sprintf( __( 'The %s Layout has Views relying on it. It will need to be manually deleted.', 'it-l10n-Builder-Parker' ), $description ) );
							else if ( $layout === $this->_options['default'] )
								ITUtility::show_error_message( sprintf( __( 'The %s Layout is the Default Layout. It will need to be manually deleted.', 'it-l10n-Builder-Parker' ), $description ) );
						}
						else {
							ITUtility::show_error_message( __( 'You must select a replacement layout.', 'it-l10n-Builder-Parker' ) );
							$this->_delete_layout_screen();
						...
						ITUtility::show_error_message( __( 'The replacement layout chosen is unavailable. Please select a different replacement layout.', 'it-l10n-Builder-Parker' ) );
						$this->_delete_layout_screen();
			...
			if ( $bulk ) {
				if ( count( $success ) > 1 )
					ITUtility::show_status_message( __( 'The requested Layouts have been deleted.', 'it-l10n-Builder-Parker' ) );
				else if ( ! empty( $success ) )
					ITUtility::show_status_message( sprintf( __( 'The %s Layout has been deleted.', 'it-l10n-Builder-Parker' ), $success[0] ) );
...
<?php
		...
		function _delete_layout_screen() {
			...
			if ( count( $this->_options['layouts'] ) < 2 ) {
				ITUtility::show_error_message( __( 'There must be at least one layout. This layout can only be removed after adding additional layouts.', 'it-l10n-Builder-Parker' ) );
				
				echo '<p class="submit">';
				$form->add_submit( 'cancel', array( 'value' => __( 'Cancel', 'it-l10n-Builder-Parker' ), 'class' => 'button-secondary', 'onclick' => 'it_dialog_remove();' ) );
				echo '</p>';
	...
	<?php $form->start_form(); ?>
		<?php if ( ( $data['total_num_views'] > 0 ) || ( $this->_options['default'] === $layout ) ) : ?>
			<?php if ( $data['total_num_views'] > 0 ) : ?>
				<div><?php _e( 'This layout is currently being used by:', 'it-l10n-Builder-Parker' ); ?></div>
				<?php
					$views = array( 'num_page_views' => _x( 'Page', 'view', 'it-l10n-Builder-Parker' ), 'num_post_views' => _x( 'Post', 'view', 'it-l10n-Builder-Parker' ), 'num_views' => _x( 'View', 'view', 'it-l10n-Builder-Parker' ) );
			...
			<?php if ( $this->_options['default'] === $layout ) : ?>
				<div><?php _e( 'This layout is the default layout. The selected replacement will be set as the new default.', 'it-l10n-Builder-Parker' ); ?></div>
				<br />
			<?php endif; ?>
			
			<div><?php _e( 'Please select a layout to use as a replacement:', 'it-l10n-Builder-Parker' ); ?></div>
			<div><?php $form->add_drop_down( 'replacement_layout', array( 'value' => $layouts, 'name' => 'replacement_layout' ) ); ?></div>
		<?php else : ?>
			<div><?php printf( __( 'Please confirm that you would like to delete the <strong>%s</strong> layout.', 'it-l10n-Builder-Parker' ), $this->_options['layouts'][$layout]['description'] ); ?></div>
		<?php endif; ?>
		
		<p class="submit">
			<?php $form->add_submit( 'delete_layout', array( 'value' => __( 'Delete', 'it-l10n-Builder-Parker' ), 'class' => 'button-primary' ) ); ?>
			<?php $form->add_submit( 'cancel', array( 'value' => __( 'Cancel', 'it-l10n-Builder-Parker' ), 'class' => 'button-secondary', 'onclick' => 'it_dialog_remove();' ) ); ?>
		</p>
...
<?php
		...
		function _duplicateLayout() {
			...
			if ( empty( $description ) ) {
				ITUtility::show_error_message( __( 'You must supply a name for the duplicated layout.', 'it-l10n-Builder-Parker' ) );
				$this->_duplicateLayoutScreen();
				return;
			}
			foreach ( (array) $this->_options['layouts'] as $layout ) {
				if ( $description === $layout['description'] ) {
					ITUtility::show_error_message( __( 'A layout with that name already exists. Please enter a different name.', 'it-l10n-Builder-Parker' ) );
					$this->_duplicateLayoutScreen();
					return;
				}
			}
			...
			if ( isset( $layout['hide_widgets'] ) && ( 'yes' === $layout['hide_widgets'] ) ) {
				$hide_widgets_text = '<strong>' . __( 'Yes', 'it-l10n-Builder-Parker' ) . '</strong>';
				$hide_widgets_link_title = __( "Show this layout's widget areas in the Widgets editor", 'it-l10n-Builder-Parker' );
				$hide_widgets_link_text = __( 'Show widget areas', 'it-l10n-Builder-Parker' );
			}
			else {
				$hide_widgets_text = 'No';
				$hide_widgets_link_title = __( "Hide this layout's widget areas from the Widgets editor", 'it-l10n-Builder-Parker' );
				$hide_widgets_link_text = __( 'Hide widget areas', 'it-l10n-Builder-Parker' );
			}
			...
			if ( builder_theme_supports( 'builder-extensions' ) ) {
				...
				$extension_title = __( 'This layout does not use an Extension', 'it-l10n-Builder-Parker' );
				$extension_text = '&nbsp;';
				...
				if ( ! empty( $layout['extension'] ) ) {
					...
					if ( ! empty( $layout['disable_style'] ) && ( 'yes' === $layout['disable_style'] ) ) {
						$extension_title = sprintf( __( 'Uses only the %s extension for styling', 'it-l10n-Builder-Parker' ), $extension_text );
						$extension_text = '<strong>' . __( $extension_text, 'it-l10n-Builder-Parker' ) . '</strong>';
					}
					else
						$extension_title = sprintf( __( 'Uses the %s extension to modify the default theme styling', 'it-l10n-Builder-Parker' ), $extension_text );
				}
			}
			
			$default_title_description = __( 'The default layout is used for all views that don\'t have a specific view set. New layouts inherit the default layout\'s background options.', 'it-l10n-Builder-Parker' );
			$layout_width_text = $layout['width'];
			$hide_widgets_description = __( 'Widget areas for layouts can be hidden from the Widgets editor to make management of widgets easier.', 'it-l10n-Builder-Parker' );
			$modify_layout_settings_title = __( 'Modify Layout Settings', 'it-l10n-Builder-Parker' );
			$edit_link_text = __( 'Edit', 'it-l10n-Builder-Parker' );
			$manage_widgets_title = __( 'Manage Widgets for this Layout', 'it-l10n-Builder-Parker' );
			$manage_widgets_link_text = __( 'Widgets', 'it-l10n-Builder-Parker' );
			$duplicate_layout_title = __( 'Duplicate Layout', 'it-l10n-Builder-Parker' );
			$duplicate_layout_text = __( 'Duplicate', 'it-l10n-Builder-Parker' );
			$delete_layout_title = __( 'Delete Layout', 'it-l10n-Builder-Parker' );
			$delete_layout_text = __( 'Delete', 'it-l10n-Builder-Parker' );
			$page_views_title = sprintf( __( 'The number of pages using %s as the Custom Layout', 'it-l10n-Builder-Parker' ), $description );
			$post_views_title = sprintf( __( 'The number of posts using %s as the Custom Layout', 'it-l10n-Builder-Parker' ), $description );
			$views_title = sprintf( __( 'The number of views using %s', 'it-l10n-Builder-Parker' ), $description );
...
?>
	<?php $form->start_form(); ?>
		<div><?php printf( __( 'Duplicating <strong>%s</strong>.', 'it-l10n-Builder-Parker' ), $this->_options['layouts'][$layout]['description'] ); ?></div>
		<br />
		...
		<p class="submit">
			<?php $form->add_submit( 'duplicate_layout', array( 'value' => __( 'Create Duplicate', 'it-l10n-Builder-Parker' ), 'class' => 'button-primary' ) ); ?>
			<?php $form->add_submit( 'cancel', array( 'value' => __( 'Cancel', 'it-l10n-Builder-Parker' ), 'class' => 'button-secondary', 'onclick' => 'it_dialog_remove();' ) ); ?>
		</p>
...
<?php
		...
		function _setDefaultLayout() {
			...
			$set_default_link = "<div class='row-actions'><a href='" . ITDialog::get_link( "{$this->_self_link}&set_default_layout_screen=$original_default" ) . "' class='it-dialog' title='" . __( 'Set this layout as the default', 'it-l10n-Builder-Parker' ) . "'>";
			$set_default_link .= __( 'Set as default', 'it-l10n-Builder-Parker' );
...
?>
	<script type="text/javascript">
		...
		win.jQuery("#entry-<?php echo $layout; ?> .set_default").html("<strong><?php _e( 'Yes', 'it-l10n-Builder-Parker' ); ?></strong>");
...
?>
	<?php $form->start_form(); ?>
		<div><?php printf( __( 'Please confirm that you would like to set <strong>%s</strong> as the default layout.', 'it-l10n-Builder-Parker' ), $this->_options['layouts'][$layout]['description'] ); ?></div>
		
		<p class="submit">
			<?php $form->add_submit( 'set_default_layout', array( 'value' => __( 'Set as Default', 'it-l10n-Builder-Parker' ), 'class' => 'button-primary' ) ); ?>
			<?php $form->add_submit( 'cancel', array( 'value' => __( 'Cancel', 'it-l10n-Builder-Parker' ), 'class' => 'button-secondary', 'onclick' => 'it_dialog_remove();' ) ); ?>
		</p>
...
?>
	<?php $form->start_form(); ?>
		<table style="border-spacing:0px 10px;">
			<h1><?php _e( 'Select a module to add to the layout', 'it-l10n-Builder-Parker' ); ?></h1>
		...
		<p class="submit">
			<?php $form->add_submit( 'add_module', array( 'value' => __( 'Add Module', 'it-l10n-Builder-Parker' ), 'class' => 'button-primary' ) ); ?>
			<?php $form->add_submit( 'cancel', array( 'value' => __( 'Cancel', 'it-l10n-Builder-Parker' ), 'class' => 'button-secondary', 'onclick' => 'it_dialog_remove();' ) ); ?>
		</p>
	<?php $form->end_form(); ?>
	...
	<script type="text/javascript">
		...
		jQuery( function() {
			it_dialog_add_form_submission_message( "<?php echo esc_js( __( 'Creating module.', 'it-l10n-Builder-Parker' ) ); ?>" );
		} );
	</script>
<?php
...
?>
	<div class="wrap">
		<?php $form->start_form(); ?>
			...
			<p><?php _e( 'Builder\'s Layouts allow you to change the structure of your site. For details, click the "Help" button at the top-right.', 'it-l10n-Builder-Parker' ); ?></p>
			...
			<div class="tablenav">
				<div class="alignleft actions">
					...
					<?php $form->add_submit( 'add_layout', array( 'value' => __( 'Create Layout', 'it-l10n-Builder-Parker' ), 'class' => 'button-secondary add' ) ); ?>
				</div>
			...
			<table id="layouts" class="widefat fixed" cellspacing="0">
				<thead>
					...
					<tr class="thead">
						...
						<th><?php _e( 'Layout Description', 'it-l10n-Builder-Parker' ); ?></th>
						<th><?php _e( 'Default Layout', 'it-l10n-Builder-Parker' ); ?> <?php ITUtility::add_tooltip( __( 'The default layout is used for all views that don\'t have a specific view set.', 'it-l10n-Builder-Parker' ) ); ?></th>
						<th><?php _e( 'Width', 'it-l10n-Builder-Parker' ); ?> <?php ITUtility::add_tooltip( __( 'The width of the Layout in pixels.', 'it-l10n-Builder-Parker' ) ); ?></th>
						<?php if ( builder_theme_supports( 'builder-extensions' ) ) : ?>
							<th><?php _e( 'Extension', 'it-l10n-Builder-Parker' ); ?> <?php ITUtility::add_tooltip( __( 'Listings in bold override the default style.css.', 'it-l10n-Builder-Parker' ) ); ?></th>
						<?php endif; ?>
						<th><?php _e( 'Hide Widget Areas', 'it-l10n-Builder-Parker' ); ?> <?php ITUtility::add_tooltip( __( 'Widget areas for layouts can be hidden from the Widgets editor to make management of widgets easier.', 'it-l10n-Builder-Parker' ) ); ?></th>
						<th><?php _e( 'Pages', 'it-l10n-Builder-Parker' ); ?> <?php ITUtility::add_tooltip( __( 'The number of pages using the layout as the Custom Layout', 'it-l10n-Builder-Parker' ) ); ?></th>
						<th><?php _e( 'Posts', 'it-l10n-Builder-Parker' ); ?> <?php ITUtility::add_tooltip( __( 'The number of posts using the layout as the Custom Layout', 'it-l10n-Builder-Parker' ) ); ?></th>
						<th><?php _e( 'Views', 'it-l10n-Builder-Parker' ); ?> <?php ITUtility::add_tooltip( __( 'The number of views using the layout', 'it-l10n-Builder-Parker' ) ); ?></th>
					</tr>
				...
				<tbody>
					...
					<?php foreach ( (array) $this->_options['layouts'] as $id => $layout ) : ?>
						<tr id="entry-<?php echo $id; ?>"<?php echo $class; ?>>
							...
							<td>
								<strong><a href="<?php echo $this->_self_link; ?>&layout=<?php echo $id; ?>" title="<?php _e( 'Modify Layout Settings', 'it-l10n-Builder-Parker' ); ?>"><?php echo $layout['description']; ?></a></strong><br />
								<div class="row-actions">
									<span class="edit"><a href="<?php echo $this->_self_link; ?>&layout=<?php echo $id; ?>" title="<?php _e( 'Modify Layout Settings', 'it-l10n-Builder-Parker' ); ?>"><?php _e( 'Edit', 'it-l10n-Builder-Parker' ); ?></a> | </span>
									
									<?php if ( current_user_can( 'edit_theme_options' ) ) : ?>
										<span class="manage-widgets"><a href="<?php echo admin_url( 'widgets.php?builder_layout_id=' . $id ); ?>" title="<?php _e( 'Manage Widgets for this Layout', 'it-l10n-Builder-Parker' ); ?>"><?php _e( 'Widgets', 'it-l10n-Builder-Parker' ); ?></a> | </span>
									<?php endif; ?>
									
									<span class="duplicate"><a href="<?php echo ITDialog::get_link( "{$this->_self_link}&duplicate_layout_screen=$id", array( 'width' => '250' ) ); ?>" class="it-dialog" title="<?php _e( 'Duplicate Layout', 'it-l10n-Builder-Parker' ); ?>"><?php _e( 'Duplicate', 'it-l10n-Builder-Parker' ); ?></a> | </span>
									<span class="delete"><a href="<?php echo ITDialog::get_link( "{$this->_self_link}&delete_layout_screen=$id" ); ?>" class="it-dialog" title="<?php _e( 'Delete Layout', 'it-l10n-Builder-Parker' ); ?>"><?php _e( 'Delete', 'it-l10n-Builder-Parker' ); ?></a></span>
								</div>
							</td>
							<td class="set_default" title="<?php _e( 'The default layout is used for all views that don\'t have a specific view set.', 'it-l10n-Builder-Parker' ); ?>">
								<?php if ( $this->_options['default'] == $id ) : ?>
									<strong><?php _e( 'Yes', 'it-l10n-Builder-Parker' ); ?></strong>
								<?php else : ?>
									<div class="row-actions">
										<a href="<?php echo ITDialog::get_link( "{$this->_self_link}&set_default_layout_screen=$id" ); ?>" class="it-dialog" title="<?php _e( 'Set this layout as the default', 'it-l10n-Builder-Parker' ); ?>"><?php _e( 'Set as default', 'it-l10n-Builder-Parker' ); ?></a>
									</div>
								<?php endif; ?>
							</td>
							...
							<?php if ( builder_theme_supports( 'builder-extensions' ) ) : ?>
								<?php
									$extension_title = __( 'This layout does not use a Extension', 'it-l10n-Builder-Parker' );
									$extension_text = '&nbsp;';
							...
							<?php endif; ?>
							<td class="widget_areas_hidden" title="<?php _e( 'Widget areas for layouts can be hidden from the Widgets editor to make management of widgets easier.', 'it-l10n-Builder-Parker' ); ?>">
								<?php if ( isset( $layout['hide_widgets'] ) && ( 'yes' === $layout['hide_widgets'] ) ) : ?>
									<strong><?php _e( 'Yes', 'it-l10n-Builder-Parker' ); ?></strong>
									<div class="row-actions">
										<a href="<?php echo $this->_self_link; ?>&show_widget_areas=<?php echo $id; ?>" title="<?php _e( 'Show this layout\'s widget areas in the Widgets editor', 'it-l10n-Builder-Parker' ); ?>"><?php _e( 'Show widget areas', 'it-l10n-Builder-Parker' ); ?></a>
									</div>
								<?php else : ?>
									<?php _e( 'No', 'it-l10n-Builder-Parker' ); ?>
									<div class="row-actions">
										<a href="<?php echo $this->_self_link; ?>&hide_widget_areas=<?php echo $id; ?>" title="<?php _e( 'Hide this layout\'s widget areas from the Widgets editor', 'it-l10n-Builder-Parker' ); ?>"><?php _e( 'Hide widget areas', 'it-l10n-Builder-Parker' ); ?></a>
									</div>
								<?php endif; ?>
							</td>
							<td class="num_page_views" title="<?php printf( __( 'The number of pages using %s as the Custom Layout', 'it-l10n-Builder-Parker' ), $layout['description'] ); ?>">
								<?php echo $layout['num_page_views']; ?>
							</td>
							<td class="num_post_views" title="<?php printf( __( 'The number of posts using %s as the Custom Layout', 'it-l10n-Builder-Parker' ), $layout['description'] ); ?>">
								<?php echo $layout['num_post_views']; ?>
							</td>
							<td class="num_views" title="<?php printf( __( 'The number of views using %s', 'it-l10n-Builder-Parker' ), $layout['description'] ); ?>">
...
?>
	<?php $form->start_form(); ?>
		<div><?php printf( __( 'Please confirm that you would like to remove the view customization for <strong>%s</strong>.', 'it-l10n-Builder-Parker' ), $view['name'] ); ?></div>
		
		<p class="submit">
			<?php $form->add_submit( 'delete_view', array( 'value' => __( 'Remove', 'it-l10n-Builder-Parker' ), 'class' => 'button-primary' ) ); ?>
			<?php $form->add_submit( 'cancel', array( 'value' => __( 'Cancel', 'it-l10n-Builder-Parker' ), 'class' => 'button-secondary', 'onclick' => 'it_dialog_remove();' ) ); ?>
		</p>
...
<?php
		...
		function _modify_view() {
			...
			if ( ! is_array( $available_views[$_POST['view']] ) ) {
				ITUtility::show_error_message( __( 'Received bad data. Either the requested Layout or View is missing.', 'it-l10n-Builder-Parker' ) );
			...
			if ( isset( $_POST['extension'] ) ) {
				$extension_rules = array(
					''                      => __( 'Use the Active Extension', 'it-l10n-Builder-Parker' ),
					'//DISABLE_EXTENSION//' => __( 'Disable the Active Extension', 'it-l10n-Builder-Parker' ),
				);
...
<?php
		...
		function _modify_view_screen( $errors = array() ) {
			...
			$layouts[__( 'Rules', 'it-l10n-Builder-Parker' )] = array(
				'//INHERIT//' => __( 'Use this View\'s Active Layout', 'it-l10n-Builder-Parker' ),
				''            => __( 'Use the site\'s Default Layout', 'it-l10n-Builder-Parker' ),
			);
			
			$layout_index_name = __( 'Layouts', 'it-l10n-Builder-Parker' );
			...
			if ( ! isset( $this->_options['views']['is_category'] ) || ( isset( $view['view_id'] ) && ( 'is_category' === $view['view_id'] ) ) ) {
				$category_options['__optgroup_1'][] = __( 'All Categories', 'it-l10n-Builder-Parker' );
			}
			...
			if ( ! isset( $this->_options['views']['is_tag'] ) || ( isset( $view['view_id'] ) && ( 'is_tag' === $view['view_id'] ) ) ) {
				$tag_options['__optgroup_1'][] = __( 'All Tags', 'it-l10n-Builder-Parker' );
			}
			...
			if ( ! isset( $this->_options['views']['is_author'] ) || ( isset( $view['view_id'] ) && ( 'is_author' === $view['view_id'] ) ) ) {
				$author_options['__optgroup_1'][] = __( 'All Authors', 'it-l10n-Builder-Parker' );
			}
			...
			if ( builder_theme_supports( 'builder-extensions' ) ) {
				...
				$extension_options[__( 'Rules', 'it-l10n-Builder-Parker' )] = array(
					''                      => __( 'Use the Active Extension', 'it-l10n-Builder-Parker' ),
					'//DISABLE_EXTENSION//' => __( 'Disable the Active Extension', 'it-l10n-Builder-Parker' ),
				);
				
				$extension_options[__( 'Extensions', 'it-l10n-Builder-Parker' )] = apply_filters( 'builder_get_extensions_with_names', array() );
				...
				foreach ( $extensions_data as $extension => $extension_data ) {
					...
					if ( $extension_data['disable_theme_style'] )
						$description .= __( '<p><strong>Notice:</strong> This Extension replaces theme styling with its own.</p>', 'it-l10n-Builder-Parker' );
...
?>
	<?php $form->start_form(); ?>
		<h1><?php echo ( empty( $_REQUEST['original_view'] ) ) ? __( 'Add View', 'it-l10n-Builder-Parker' ) : __( 'Modify View', 'it-l10n-Builder-Parker' ); ?></h1>
		
		<table class="valign-top">
			<tr><td><?php _e( 'View', 'it-l10n-Builder-Parker' ); ?></td>
				<td>
					<?php $form->add_drop_down( 'view', $views ); ?>
					<div id="view-description"></div>
				</td>
			</tr>
			<tr id="tag-options" style="display:none;"><td><?php _e( 'Tag', 'it-l10n-Builder-Parker' ); ?></td>
				<td>
					<?php $form->add_drop_down( 'tag_id', $tag_options ); ?>
				</td>
			</tr>
			<tr id="category-options" style="display:none;"><td><?php _e( 'Category', 'it-l10n-Builder-Parker' ); ?></td>
				<td>
					<?php $form->add_drop_down( 'category_id', $category_options ); ?>
				</td>
			</tr>
			<tr id="author-options" style="display:none;"><td><?php _e( 'Author', 'it-l10n-Builder-Parker' ); ?></td>
				<td>
					<?php $form->add_drop_down( 'author_id', $author_options ); ?>
				</td>
			</tr>
			<tr><td><?php _e( 'Layout', 'it-l10n-Builder-Parker' ); ?></td>
				<td>
					<?php $form->add_drop_down( 'layout', $layouts ); ?>
					<?php ITUtility::add_tooltip( __( 'The Active Layout is the Layout that would be used if this View did not exist.', 'it-l10n-Builder-Parker' ) ); ?>
				</td>
			</tr>
			<?php if ( builder_theme_supports( 'builder-extensions' ) ) : ?>
				<tr><td><?php _e( 'Extension', 'it-l10n-Builder-Parker' ); ?></td>
					<td>
						<?php $form->add_drop_down( 'extension', $extension_options ); ?>
						<?php ITUtility::add_tooltip( __( 'The Active Extension is the Extension that would be used if this View did not exist.', 'it-l10n-Builder-Parker' ) ); ?>
		...
		<p class="submit">
			<?php $save_desc = ( empty( $_REQUEST['original_view'] ) ) ? __( 'Add', 'it-l10n-Builder-Parker' ) : __( 'Update', 'it-l10n-Builder-Parker' ); ?>
...
?>
	<div class="wrap">
		<?php $form->start_form(); ?>
			...
			<p><?php _e( 'Views allow you to apply specific Layouts to specific parts of your site. For details, click the "Help" button at the top-right.', 'it-l10n-Builder-Parker' ); ?></p>
			
			<div id="no-views-container">
				<br />
				<p><?php _e( 'Your site currently does not have any Views configured. Please click the "Add View" button below to apply a specific Layout to a site View.', 'it-l10n-Builder-Parker' ); ?></p>
				<a href="<?php echo $add_link; ?>" class="it-dialog button-secondary link-secondary"><?php _e( 'Add View', 'it-l10n-Builder-Parker' ); ?></a>
			</div>
			...
			<div id="views-container">
				...
				<table id="views-table" class="widefat fixed">
					<thead>
						<tr class="thead">
							<th><?php _e( 'Site View', 'it-l10n-Builder-Parker' ); ?></th>
							<th><?php _e( 'Site View Description', 'it-l10n-Builder-Parker' ); ?></th>
							<th title=""><?php _e( 'Layout', 'it-l10n-Builder-Parker' ); ?></th>
							<?php if ( builder_theme_supports( 'builder-extensions' ) ) : ?>
								<th><?php _e( 'Extension', 'it-l10n-Builder-Parker' ); ?></th>
							<?php endif; ?>
						</tr>
					</thead>
					<tfoot>
					...
					<tbody>
						...
						<?php foreach ( (array) $views as $view_id => $view ) : ?>
							<tr class="view-entry" id="view-<?php echo $this->_parse_view_id( $view_id ); ?>"<?php echo $class; ?>>
								<td>
									<strong><a class="view-name it-dialog" href="<?php echo ITDialog::get_link( "{$this->_self_link}&modify_view_screen=1&original_view=$view_id", "modal=true&max-width=400" ); ?>" title="<?php _e( 'Modify View', 'it-l10n-Builder-Parker' ); ?>"><?php echo $view['name']; ?></a></strong><br />
									<div class="row-actions">
										<span class="edit"><a href="<?php echo ITDialog::get_link( "{$this->_self_link}&modify_view_screen=1&original_view=$view_id", "modal=true&max-width=400" ); ?>" class="it-dialog" title="Modify View"><?php _e( 'Edit', 'it-l10n-Builder-Parker' ); ?></a> | </span>
										<span class="delete"><a href="<?php echo ITDialog::get_link( "{$this->_self_link}&delete_view_screen=$view_id" ); ?>" class="it-dialog" title="Remove View Customization"><?php _e( 'Remove', 'it-l10n-Builder-Parker' ); ?></a></span>
									</div>
								</td>
								<td><?php echo $view['description']; ?></td>
								<td class="view-layout">
									<?php if ( isset( $this->_options['layouts'][$view['layout']] ) ) : ?>
										<a href="<?php echo $this->_tabless_self_link; ?>&editor_tab=layouts&layout=<?php echo $view['layout']; ?>" title="<?php _e( 'Modify Layout', 'it-l10n-Builder-Parker' ); ?>">
				...
				<div class="tablenav">
					<div class="alignleft actions">
						<a href="<?php echo $add_link; ?>" class="button-secondary link-secondary it-dialog"><?php _e( 'Add View', 'it-l10n-Builder-Parker' ); ?></a>
					</div>
			...
			<div id="new-view-container" style="display:none;">
				<table>
					<tr class="view-entry" id="view-%parsed_view_id%" style="display:none;">
						<td>
							<strong><a class="view-name it-dialog" href="<?php echo ITDialog::get_link( "{$this->_self_link}&modify_view_screen=1&original_view=%view_id%", "modal=true" ); ?>" title="<?php _e( 'Modify View', 'it-l10n-Builder-Parker' ); ?>">%view_name%</a></strong><br />
							<div class="row-actions">
								<span class="edit"><a href="<?php echo ITDialog::get_link( "{$this->_self_link}&modify_view_screen=1&original_view=%view_id%", "modal=true" ); ?>" class="it-dialog" title="Modify View"><?php _e( 'Edit', 'it-l10n-Builder-Parker' ); ?></a> | </span>
								<span class="delete"><a href="<?php echo ITDialog::get_link( "{$this->_self_link}&delete_view_screen=%view_id%" ); ?>" class="it-dialog" title="Remove View Customization"><?php _e( 'Remove', 'it-l10n-Builder-Parker' ); ?></a></span>
							</div>
						</td>
						<td>%view_description%</td>
						<td class="" title="">
							<a href="<?php echo $this->_self_link; ?>&layout=%layout%" title="<?php _e( 'Modify Layout', 'it-l10n-Builder-Parker' ); ?>">
								%layout_description%
							</a>
						</td>
						<?php if ( builder_theme_supports( 'builder-extensions' ) ) : ?>
...
<?php
		...
		function _save_layout() {
			...
			if ( empty( $layout['description'] ) ) {
				ITUtility::show_error_message( __( 'You must supply a Name.', 'it-l10n-Builder-Parker' ) );
				$error = true;
			}
			
			if ( empty( $layout['width'] ) ) {
				ITUtility::show_error_message( __( 'Please supply a Width.', 'it-l10n-Builder-Parker' ) );
				$error = true;
			}
			else if ( (string) intval( $layout['width'] ) != (string) $layout['width'] ) {
				ITUtility::show_error_message( __( 'The Width must be an integer number.', 'it-l10n-Builder-Parker' ) );
				$error = true;
			}
			else if ( $layout['width'] < 50 ) {
				ITUtility::show_error_message( __( 'The Width must be at least 50 pixels. Please increase the Width.', 'it-l10n-Builder-Parker' ) );
				$error = true;
			}
			...
			if ( ! empty( $_REQUEST['add_layout'] ) ) {
				foreach ( (array) $this->_options['layouts'] as $ex_layout ) {
					if ( strtolower( $layout['description'] ) === strtolower( $ex_layout['description'] ) ) {
						ITUtility::show_error_message( __( 'A layout with that Name already exists. Please choose a unique name.', 'it-l10n-Builder-Parker' ) );
				...
				ITUtility::show_status_message( sprintf( __( '%s created', 'it-l10n-Builder-Parker' ), $layout['description'] ) );
			}
			else if ( ! empty( $_REQUEST['layout'] ) ) {
				$this->_options['layouts'][$_REQUEST['layout']] = $layout;
				
				ITUtility::show_status_message( sprintf( __( '%s updated', 'it-l10n-Builder-Parker' ), $layout['description'] ) );
			}
		...
		function _modify_layout() {
			...
			$layout_widths = array(
				'600'		=> __( 'Narrow (600 pixels)', 'it-l10n-Builder-Parker' ),
				'780'		=> __( 'Medium (780 pixels)', 'it-l10n-Builder-Parker' ),
				'960'		=> __( 'Wide (960 pixels)', 'it-l10n-Builder-Parker' ),
			);
			...
			$layout_widths['custom'] = __( 'Custom...', 'it-l10n-Builder-Parker' );
			
			
			if ( builder_theme_supports( 'builder-extensions' ) ) {
				$extensions_data = apply_filters( 'builder_get_extensions_data', array() );
				
				$extensions = array( '' => __( '-- No Extension --', 'it-l10n-Builder-Parker' ) );
...
?>
	<div class="wrap">
		...
		<?php $form->start_form(); ?>
			<h3 class="title"><?php _e( 'Settings', 'it-l10n-Builder-Parker' ); ?></h3>
			<table class="form-table">
				<tr><th scope="row"><label for="description"><?php _e( 'Name', 'it-l10n-Builder-Parker' ); ?></label></th>
					<td>
						<?php $form->add_text_box( 'description', array( 'size' => '15', 'maxlength' => '15' ) ); ?>
						<?php ITUtility::add_tooltip( __( 'The name helps identify this Layout\'s widget areas. Choose a descriptive, short name.', 'it-l10n-Builder-Parker' ) ); ?>
					</td>
				</tr>
				<tr><th scope="row"><label for="width"><?php _e( 'Width', 'it-l10n-Builder-Parker' ); ?></label></th>
					<td>
						...
						<div id="layout-width-custom" style="display:none;">
							<label>
								<?php _e( 'Custom Width', 'it-l10n-Builder-Parker' ); ?>
								<?php $form->add_text_box( 'custom_width', array( 'size' => '4', 'maxlength' => '5' ) ); ?>
								<?php _e( 'pixels', 'it-l10n-Builder-Parker' ); ?>
							</label>
						</div>
					</td>
				</tr>
				<?php if ( builder_theme_supports( 'builder-extensions' ) ) : ?>
					<tr><th scope="row"><label for="extension"><?php _e( 'Extension', 'it-l10n-Builder-Parker' ); ?></label></th>
						<td>
							<?php $form->add_drop_down( 'extension', $extensions ); ?>
							<?php ITUtility::add_tooltip( __( 'Extensions can provide additional code that changes the content, provides additional features, or modifies the styling of the Layout.<br /><br />You can find Extensions in your theme\'s directory inside a directory named "extensions".', 'it-l10n-Builder-Parker' ) ); ?>
							
							<div id="extension-details"></div>
						</td>
					</tr>
				<?php endif; ?>
				<tr><th scope="row"><label for="hide_widgets"><?php _e( 'Hide Widget Areas', 'it-l10n-Builder-Parker' ); ?></label></th>
					<td>
						<?php $form->add_drop_down( 'hide_widgets', array( 'no' => __( 'No', 'it-l10n-Builder-Parker' ), 'yes' => __( 'Yes', 'it-l10n-Builder-Parker' ) ) ); ?>
						<?php ITUtility::add_tooltip( __( 'Use this option to hide this Layout\'s widget areas from the <strong>Appearance &gt; Widgets</strong> editor. This makes it easier to work with other Layout\'s widget areas.', 'it-l10n-Builder-Parker' ) ); ?>
					</td>
				</tr>
			...
			<table class="form-table layout-modules">
				<tr class="add-module-help"><td colspan="2"><?php _e( 'In order to start building your layout, please click the Add Module link below.', 'it-l10n-Builder-Parker' ); ?></td></tr>
				<?php
					$position = 1;
					$max_id = 0;
			...
			<p class="submit">
				<?php $form->add_submit( 'save', array( 'value' => __( 'Save Layout', 'it-l10n-Builder-Parker' ), 'class' => 'button-primary' ) ); ?>
				<?php $form->add_submit( 'save_and_continue', array( 'value' => __( 'Save Layout and Continue Editing', 'it-l10n-Builder-Parker' ), 'class' => 'button-secondary' ) ); ?>
				<?php $form->add_submit( 'cancel', array( 'value' => __( 'Cancel', 'it-l10n-Builder-Parker' ), 'class' => 'button-secondary cancel' ) ); ?>
			</p>
