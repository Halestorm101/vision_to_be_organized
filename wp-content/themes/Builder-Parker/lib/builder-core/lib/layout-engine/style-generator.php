		...
		if ( builder_theme_supports( 'builder-disable-stylesheet-cache' ) ) {
			...
			if ( ! empty( $this->layout['stylesheet_file'] ) ) {
				if ( is_string( $this->layout['stylesheet_file'] ) && file_exists( $this->layout['stylesheet_file'] ) ) {
					...
					echo "\n<!--\n\t" . __( 'Builder Error: Unable to find a writable location to store the generated stylesheet. Check directory permissions in the wp-content/uploads directory.', 'it-l10n-Builder-Parker' ) . "\n";
					echo "\t" . sprintf( __( 'Error Message (%s): %s', 'it-l10n-Builder-Parker' ), $this->layout['stylesheet_file']->get_error_code(), $this->layout['stylesheet_file']->get_error_message() ) . "\n";
					echo "-->\n";
				}
			}
