...
if ( ! class_exists( 'LayoutModuleWidgetBar' ) ) {
	class LayoutModuleWidgetBar extends LayoutModule {
		...
		function __construct() {
			$this->_name = _x( 'Widget Bar', 'module', 'it-l10n-Builder-Parker' );
			$this->_description = __( 'This module can contain one to five widget locations that are displayed as columns.', 'it-l10n-Builder-Parker' );
		...
		function _validate( $result ) {
			if ( 'custom' === $_POST['widget_percents'] ) {
				...
				if ( count( explode( ',', $_POST['custom_widget_percents'] ) ) != $_POST['type'] )
					$result['errors'][] = sprintf( __( 'You must supply %s custom widget width percentages separated by commas', 'it-l10n-Builder-Parker' ), $_POST['type'] );
				else if ( preg_match( '/[^0-9,\.]/', $result['data']['custom_widget_percents'] ) ) {
					$result['errors'][] = __( 'Only numbers can be used for custom widget width percentages', 'it-l10n-Builder-Parker' );
				}
				else if ( ( array_sum( explode( ',', $result['data']['custom_widget_percents'] ) ) > 102 ) || ( array_sum( explode( ',', $result['data']['custom_widget_percents'] ) ) < 98 ) )
					$result['errors'][] = __( 'The sum of the custom widget width percentages must equal 100', 'it-l10n-Builder-Parker' );
				else {
					foreach ( (array) explode( ',', $result['data']['custom_widget_percents'] ) as $percent ) {
						if ( $percent < 1 ) {
							$result['errors'][] = __( 'The minimum percentage width for a widget area is 1%. Please ensure that all of your custom widget width percentages are 1% or greater.', 'it-l10n-Builder-Parker' );
							break;
						}
					}
				}
		...
		function _get_default_percents() {
			$default_percents = array(
				'1' => array(
					'100' => __( '100%', 'it-l10n-Builder-Parker' ),
				),
				'2' => array(
					'50,50'  => __( '50% / 50%', 'it-l10n-Builder-Parker' ),
					'80,20'  => __( '80% / 20%', 'it-l10n-Builder-Parker' ),
					'20,80'  => __( '20% / 80%', 'it-l10n-Builder-Parker' ),
					'70,30'  => __( '70% / 30%', 'it-l10n-Builder-Parker' ),
					'30,70'  => __( '30% / 70%', 'it-l10n-Builder-Parker' ),
					'60,40'  => __( '60% / 40%', 'it-l10n-Builder-Parker' ),
					'40,60'  => __( '40% / 60%', 'it-l10n-Builder-Parker' ),
					'custom' => __( 'Custom...', 'it-l10n-Builder-Parker' ),
				),
				'3' => array(
					'33.333,33.333,33.333' => __( '33% / 33% / 33%', 'it-l10n-Builder-Parker' ),
					'60,20,20'             => __( '60% / 20% / 20%', 'it-l10n-Builder-Parker' ),
					'20,60,20'             => __( '20% / 60% / 20%', 'it-l10n-Builder-Parker' ),
					'20,20,60'             => __( '20% / 20% / 60%', 'it-l10n-Builder-Parker' ),
					'50,25,25'             => __( '50% / 25% / 25%', 'it-l10n-Builder-Parker' ),
					'25,50,25'             => __( '25% / 50% / 25%', 'it-l10n-Builder-Parker' ),
					'25,25,50'             => __( '25% / 25% / 50%', 'it-l10n-Builder-Parker' ),
					'custom'               => __( 'Custom...', 'it-l10n-Builder-Parker' ),
				),
				'4' => array(
					'25,25,25,25' => __( '25% / 25% / 25% / 25%', 'it-l10n-Builder-Parker' ),
					'40,20,20,20' => __( '40% / 20% / 20% / 20%', 'it-l10n-Builder-Parker' ),
					'20,40,20,20' => __( '20% / 40% / 20% / 20%', 'it-l10n-Builder-Parker' ),
					'20,20,40,20' => __( '20% / 20% / 40% / 20%', 'it-l10n-Builder-Parker' ),
					'20,20,20,40' => __( '20% / 20% / 20% / 40%', 'it-l10n-Builder-Parker' ),
					'custom'      => __( 'Custom...', 'it-l10n-Builder-Parker' ),
				),
				'5' => array(
					'20,20,20,20,20' => __( '20% / 20% / 20%/ 20% / 20%', 'it-l10n-Builder-Parker' ),
					'custom'         => __( 'Custom...', 'it-l10n-Builder-Parker' ),
				),
			);
...
?>
	<tr><td><label for="type"><?php _e( 'Columns', 'it-l10n-Builder-Parker' ); ?></label></td>
		<td>
			<?php $form->add_drop_down( 'type', $types ); ?>
			<?php ITUtility::add_tooltip( __( 'Each column becomes a widget area that you can add widgets to.', 'it-l10n-Builder-Parker' ) ); ?>
		</td>
	</tr>
	<tr style="vertical-align:top;"><td><?php _e( 'Column Widths', 'it-l10n-Builder-Parker' ); ?></td>
		<td id="widget_percents_row">
			<?php $form->add_drop_down( 'widget_percents', $start_widget_percents ); ?>
			<?php ITUtility::add_tooltip( __( 'You can make the columns wider or narrower by using these options. To get more control over the widths used, select "Custom..." to put in your own widths.', 'it-l10n-Builder-Parker' ) ); ?>
			...
			<div id="widget-percents-custom" style="display:none;">
				...
				<?php _e( 'Specify the percentage width of each widget area.', 'it-l10n-Builder-Parker' ); ?><br />
				<?php _e( 'For example, if you have three widget areas and want to separate them into 20%, 30%, and 50% widths, input <strong><code>20,30,50</code></strong> in the box above.', 'it-l10n-Builder-Parker' ); ?>
			</div>
		</td>
	</tr>
<?php
