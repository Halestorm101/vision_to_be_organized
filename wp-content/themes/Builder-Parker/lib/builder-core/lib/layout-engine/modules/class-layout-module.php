			...
			if ( true === $this->_has_sidebars ) {
				if ( isset( $_POST['sidebar_widths'] ) && ( 'custom' === $_POST['sidebar_widths'] ) ) {
					...
					if ( ( 1 === $expected_widths[$_POST['sidebar']] ) && ( 1 !== $num_widths ) )
						$result['errors'][] = __( 'You must supply a width for the sidebar', 'it-l10n-Builder-Parker' );
					if ( $num_widths != $expected_widths[$_POST['sidebar']] )
						$result['errors'][] = sprintf( __( 'You must supply %s widths separated by commas', 'it-l10n-Builder-Parker' ), $expected_widths[$_POST['sidebar']] );
					else if ( preg_match( '/[^0-9,\.]/', $result['data']['custom_sidebar_widths'] ) ) {
						$result['errors'][] = __( 'Only numbers can be used for sidebar widths', 'it-l10n-Builder-Parker' );
					}
					else if ( array_sum( explode( ',', $result['data']['custom_sidebar_widths'] ) ) > ( $_POST['layout_width'] - 200 ) )
						$result['errors'][] = __( 'You must leave at least a 200 pixel area for the module content to render in. Please reduce the size of the sidebar widths.', 'it-l10n-Builder-Parker' );
					else {
						foreach ( (array) explode( ',', $result['data']['custom_sidebar_widths'] ) as $width ) {
							if ( $width < 20 ) {
								$result['errors'][] = __( 'The minimum width for a sidebar is 20 pixels. Please ensure that all of your custom sidebar widths are 20 pixels or larger.', 'it-l10n-Builder-Parker' );
								break;
							}
						}
					}
...
?>
	<tr><td><label for="name"><?php _e( 'Name', 'it-l10n-Builder-Parker' ); ?></label></td>
		<td>
			<?php $form->add_text_box( 'name' ); ?>
			<?php ITUtility::add_tooltip( __( 'The module\'s name helps you identify specific widget locations. Descriptive, short names typically work best.', 'it-l10n-Builder-Parker' ) ); ?>
			<br /><br />
		</td>
	</tr>
<?php
			...
			if ( true === $this->_has_sidebars ) {
				$sidebars = array(
					'none'      => __( 'No Sidebars', 'it-l10n-Builder-Parker' ),
					'1_left'    => __( '1 Left', 'it-l10n-Builder-Parker' ),
					'2_left'    => __( '2 Left', 'it-l10n-Builder-Parker' ),
					'1_right'   => __( '1 Right', 'it-l10n-Builder-Parker' ),
					'2_right'   => __( '2 Right', 'it-l10n-Builder-Parker' ),
					'split'     => __( 'Split (1 Left & 1 Right)', 'it-l10n-Builder-Parker' ),
				);

				$default_widths = array(
					'none'      => array(),
					'1_left'    => array(
						'150'     => __( 'Narrow (150px)', 'it-l10n-Builder-Parker' ),
						'180'     => __( 'Standard (180px)', 'it-l10n-Builder-Parker' ),
						'200'     => __( 'Wide (200px)', 'it-l10n-Builder-Parker' ),
						'custom'  => __( 'Custom...', 'it-l10n-Builder-Parker' ),
					),
					'2_left'    => array(
						'150,150' => __( 'Narrow/Narrow (150px/150px)', 'it-l10n-Builder-Parker' ),
						'150,180' => __( 'Narrow/Standard (150px/180px)', 'it-l10n-Builder-Parker' ),
						'150,200' => __( 'Narrow/Wide (150px/200px)', 'it-l10n-Builder-Parker' ),
						'180,150' => __( 'Standard/Narrow (180px/150px)', 'it-l10n-Builder-Parker' ),
						'180,180' => __( 'Standard/Standard (180px/180px)', 'it-l10n-Builder-Parker' ),
						'180,200' => __( 'Standard/Wide (180px/200px)', 'it-l10n-Builder-Parker' ),
						'200,150' => __( 'Wide/Narrow (200px/150px)', 'it-l10n-Builder-Parker' ),
						'200,180' => __( 'Wide/Standard (200px/180px)', 'it-l10n-Builder-Parker' ),
						'200,200' => __( 'Wide/Wide (200px/200px)', 'it-l10n-Builder-Parker' ),
						'custom'  => __( 'Custom...', 'it-l10n-Builder-Parker' ),
					),
				);
...
?>
	<tr><td><label for="sidebar"><?php _e( 'Sidebars', 'it-l10n-Builder-Parker' ); ?></label></td>
		<td>
			<?php $form->add_drop_down( 'sidebar', $sidebars ); ?>
			<?php ITUtility::add_tooltip( __( 'Sidebars can be added to this module so that you can have widgets to the left, right, or on both sides of the modules content.', 'it-l10n-Builder-Parker' ) ); ?>
	...
	<tr id="sidebar-widths-row">
		<td><?php _e( 'Sidebar&nbsp;Widths', 'it-l10n-Builder-Parker' ); ?></td>
		<td>
			<?php $form->add_drop_down( 'sidebar_widths', null ); ?>
			<?php ITUtility::add_tooltip( __( 'You can make the widget areas wider or narrower by using these options. To get more control over the widths used, select "Custom..." to put in your own widths.', 'it-l10n-Builder-Parker' ) ); ?>

			<div id="sidebar-widths-custom" style="display:none;">
				<?php $form->add_text_box( 'custom_sidebar_widths', array( 'size' => '10', 'maxlength' => '20' ) ); ?><br />
				<?php _e( 'Specify the width of each sidebar in pixels.', 'it-l10n-Builder-Parker' ); ?><br />
				<?php _e( 'For example, if you selected to have 2 Right sidebars and want to separate them into 170 pixel and 185 pixel widths, input <strong><code>170,185</code></strong> in the box above.', 'it-l10n-Builder-Parker' ); ?>
			</div>
		</td>
	</tr>
<?php
...
?>
	...
	<tr><td><label for="sidebar"><?php _e( 'Style', 'it-l10n-Builder-Parker' ); ?></label></td>
...
?>
	<tr><td><label for="remove_wrappers"><?php _e( 'Remove&nbsp;Wrapper&nbsp;DIVs', 'it-l10n-Builder-Parker' ); ?></label></td>
		<td>
			<?php $form->add_drop_down( 'remove_wrappers', array( '' => __( 'No', 'it-l10n-Builder-Parker' ), '1' => __( 'Yes', 'it-l10n-Builder-Parker' ) ) ); ?><?php ITUtility::add_tooltip( __( 'Enabling this option is only recommended for advanced users as this can cause layout issues if not properly handled.', 'it-l10n-Builder-Parker' ) ); ?>
		</td>
	</tr>
<?php
...
/*			$width_index = $max_width_index = false;
		...
		function render_sidebar_block( $fields, $side ) {
			...
			if ( empty( $data['sidebar_widths'] ) )
				ITError::admin_warn( 'empty_var:parameter:data[\'sidebar_widths\']', __( 'Unable to decide widths due to missing sidebar_widths value.', 'it-l10n-Builder-Parker' ) );
