	...
	function dynamic_sidebar( $index, $name = null ) {
		...
		if ( empty( $sidebar_index ) ) {
			if ( current_user_can( 'switch_themes' ) )
				printf( __( 'A problem happened with the widget area registration. This sidebar has an index of <code>%s</code> but it was unable to be found in the registered sidebars.', 'it-l10n-Builder-Parker' ), $index );
		}
		else if ( builder_identify_widget_area( dynamic_sidebar( $sidebar_index ) ) ) {
			$instance = array(
				'title'  => __( 'This is a Widget Section', 'it-l10n-Builder-Parker' ),
				'text'   => sprintf( __( 'This section is widgetized. If you would like to add content to this section, you may do so by using the Widgets panel from within your WordPress Admin Dashboard. This Widget Section is called "<strong>%s</strong>"', 'it-l10n-Builder-Parker' ), $name ),
				'filter' => 'on',
			);
