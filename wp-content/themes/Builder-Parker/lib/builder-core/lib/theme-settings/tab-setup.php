...
//		return 'This is the setup tab\'s settings.';
	...
	function add_admin_scripts() {
		...
		$translations = array(
			'confirm_dialog_text' => __( 'Using the "Use the Layouts and Views included with the theme" option will cause all of your current Layout and Views to be removed and replaced with the ones from the theme. Please confirm that you wish to do this.', 'it-l10n-Builder-Parker' ),
		);
...
/*		if ( ! empty( $_POST['layouts_import_method'] ) && in_array( $_POST['layouts_import_method'], array( 'add', 'replace' ) ) )
	...
	function _show_child_theme_options() {
		...
		$options['child_theme_name'] = sprintf( __( '%s - Custom', 'it-l10n-Builder-Parker' ), $themes[basename( get_template_directory() )]->get( 'Name' ) );
		...
		<?php $form->start_form(); ?>
			...
			<div class="it-brochure-box">
				...
				<?php if ( 'child_info' == $type ) : ?>
					<div class="it-notice"><?php printf( __( 'You are currently running a custom Builder child theme called <strong>%1$s</strong> created for the <strong>%2$s</strong> theme. All customizations should be made to this custom theme.', 'it-l10n-Builder-Parker' ), $child->get( 'Name' ), $parent->get( 'Name' ) ); ?></div>
					<br />
					
					<div class="it-input-set">
						<?php if ( 'finish' == $options['step'] ) : ?>
							<?php $form->add_submit( 'ignore', array( 'value' => __( 'Continue', 'it-l10n-Builder-Parker' ) ) ); ?>
						<?php else : ?>
							<?php $form->add_submit( 'ignore', array( 'value' => __( 'Continue to Layouts and Views Setup', 'it-l10n-Builder-Parker' ) ) ); ?>
						<?php endif; ?>
					</div>
				<?php elseif ( 'create_child' == $type ) : ?>
					<p><?php printf( __( 'To create a <a href="%1$s">child theme</a> that you can customize, choose a name below and click the <em>Create My Child Theme</em> button.', 'it-l10n-Builder-Parker' ), 'http://codex.wordpress.org/Child_Themes' ); ?></p>
					
					<div class="it-input-set">
						<label for="child_theme_name"><?php _e( 'Your child theme name', 'it-l10n-Builder-Parker' ); ?></label>
						<?php ITUtility::add_tooltip( __( 'Use a descriptive name that will help you identify your theme\'s name.', 'it-l10n-Builder-Parker' ) ); ?>
						<br />
					...
					<div class="it-input-set">
						<?php $form->add_submit( 'create_child_theme', array( 'value' => __( 'Create My Child Theme', 'it-l10n-Builder-Parker' ) ) ); ?>
						
						<?php if ( empty( $GLOBALS['builder_fresh_install'] ) ) : ?>
							<p><a class="it-skip" href="<?php echo admin_url( 'admin.php?page=theme-settings&editor_tab=setup&step=layouts_and_views' ); ?>"><?php _e( 'Skip to next step', 'it-l10n-Builder-Parker' ); ?></a></p>
						<?php else: ?>
							<p><a class="it-skip" href="<?php echo admin_url( 'admin.php?page=ithemes-builder-theme' ); ?>"><?php _e( 'Skip', 'it-l10n-Builder-Parker' ); ?></a></p>
						<?php endif; ?>
					</div>
				<?php else : ?>
					<p><?php printf( _n( 'Your site has a <a href="%1$s">child theme</a> for the %2$s theme available. As it is recommended to always run a child theme rather than the parent theme, please either activate the existing child theme or create a new one using the options below.', 'Your site has <a href="%1$s">child themes</a> for the %2$s theme available. As it is recommended to always run a child theme rather than the parent theme, please either activate an existing child theme or create a new one using the options below.', count( $child_themes ), 'it-l10n-Builder-Parker' ), 'http://codex.wordpress.org/Child_Themes', $parent->get( 'Name' ) ); ?></p>
					
					<div class="it-shrink-wrap-box">
						<label><?php printf( __( '%1$s Activate an existing child theme', 'it-l10n-Builder-Parker' ), $form->add_radio( 'child_theme_option', array( 'value' => 'activate', 'class' => 'show-hide-toggle' ) ) ); ?></label>
						<br />
						
						<label><?php printf( __( '%1$s Create a new child theme', 'it-l10n-Builder-Parker' ), $form->add_radio( 'child_theme_option', array( 'value' => 'create', 'class' => 'show-hide-toggle' ) ) ); ?></label>
						<br />
						<br />
						
						<div class="it-options-child_theme_option it-options-child_theme_option-activate">
							<p><label for="child_theme"><?php _e( 'Select the theme to activate:', 'it-l10n-Builder-Parker' ); ?></label></p>
							<?php $form->add_drop_down( 'child_theme', $child_themes ); ?>
							<br />
							<br />
							<br />
							<?php $form->add_submit( 'create_child_theme', array( 'value' => __( 'Activate My Child Theme', 'it-l10n-Builder-Parker' ) ) ); ?>
						</div>
						
						<div class="it-options-child_theme_option it-options-child_theme_option-create">
							<p>
								<label for="child_theme_name"><?php _e( 'Your child theme name', 'it-l10n-Builder-Parker' ); ?></label>
								<?php ITUtility::add_tooltip( __( 'Use a descriptive name that will help you identify your theme.', 'it-l10n-Builder-Parker' ) ); ?>
							</p>
		...
		<?php $form->start_form( array( 'id' => 'it-builder-setup' ) ); ?>
			...
			<div class="it-brochure-box">
				...
				<?php if ( $template_switch ) : ?>
					<p><?php _e( 'You are switching from a different Builder theme. Would you like to continue to use your current Layouts and Views? Or do you want to use the Layouts and Views included with this new theme?', 'it-l10n-Builder-Parker' ); ?></p>
					
					<div class="it-shrink-wrap-box">
						<label><?php printf( __( '%1$s Keep the site\'s current Layouts and Views', 'it-l10n-Builder-Parker' ), $form->add_radio( 'theme_layouts', array( 'value' => 'ignore', 'class' => 'show-hide-toggle' ) ) ); ?></label>
						<br />
						<label><?php printf( __( '%1$s Use the Layouts and Views included with the new theme', 'it-l10n-Builder-Parker' ), $form->add_radio( 'theme_layouts', array( 'value' => 'use', 'class' => 'show-hide-toggle' ) ) ); ?></label>
						<div class="it-options-theme_layouts it-options-theme_layouts-use">
							<p class="description"><?php _e( 'Important: This option will replace your current Layouts and Views with the Layouts and Views provided by the theme.', 'it-l10n-Builder-Parker' ); ?></p>
						</div>
						
	<!--				<div class="it-indent-box it-options-theme_layouts it-options-theme_layouts-use">
							<label><?php printf( __( '%1$s Keep current Layouts', 'it-l10n-Builder-Parker' ), $form->add_radio( 'layouts_import_method', 'add' ) ); ?></label>
							<br />
							<label><?php printf( __( '%1$s Remove current Layouts', 'it-l10n-Builder-Parker' ), $form->add_radio( 'layouts_import_method', 'replace' ) ); ?></label>
						</div>-->
					</div>
				<?php else : ?>
					<p><?php _e( 'Your theme provides a set of default Layouts and Views. You can update your site to match these provided Layouts and Views.', 'it-l10n-Builder-Parker' ); ?></p>
					
					<div class="it-shrink-wrap-box">
						<label><?php printf( __( '%1$s Keep the site\'s current Layouts and Views', 'it-l10n-Builder-Parker' ), $form->add_radio( 'theme_layouts', array( 'value' => 'ignore', 'class' => 'show-hide-toggle' ) ) ); ?></label>
						<br />
						<label><?php printf( __( '%1$s Use the Layouts and Views included with the theme', 'it-l10n-Builder-Parker' ), $form->add_radio( 'theme_layouts', array( 'value' => 'use', 'class' => 'show-hide-toggle' ) ) ); ?></label>
						<div class="it-options-theme_layouts it-options-theme_layouts-use">
							<p class="description"><?php _e( 'Important: This option will replace your current Layouts and Views with the Layouts and Views provided by the theme.', 'it-l10n-Builder-Parker' ); ?></p>
						</div>
