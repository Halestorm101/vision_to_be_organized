...
function builder_create_child_theme( $args = array() ) {
	...
	if ( empty( $name ) )
		$name = sprintf( __( '%s - Custom', 'it-l10n-Builder-Parker' ), $themes[basename( $parent_directory )]->get( 'Name' ) );
	...
	if ( 'copy' == $source_type ) {
		...
		$stylesheet_data['description'] = sprintf( __( 'This is a generated child theme for the %1$s theme. You should activate and modify this theme instead of %1$s. Doing so allows you to modify this child theme while allowing automatic upgrades for %1$s.', 'it-l10n-Builder-Parker' ), $stylesheet_data['name'] );
