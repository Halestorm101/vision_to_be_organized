...
class Ithemes_Updater_API {
	...
	public static function get_package_changelog( $package, $cur_version = false ) {
		...
		if ( is_wp_error( $response ) ) {
			return sprintf( __( '<p>Unable to get changelog data at this time.</p><p>Error <code>%1$s</code>: %2$s</p>', 'it-l10n-Builder-Parker' ), $response->get_error_code(), $response->get_error_message() );
		}
		
		if ( ! is_array( $response ) || ! isset( $response['body'] ) ) {
			return __( '<p>Unable to get changelog data at this time.</p><p>Error: Unrecognized response from <code>wp_remote_get</code>.</p>', 'it-l10n-Builder-Parker' );
		}
		
		if ( isset( $response['response']['code'] ) && ( '200' != $response['response']['code'] ) ) {
			return sprintf( __( '<p>Unable to get changelog data at this time.</p><p>Error code <code>%1$s</code>: %2$s</p>', 'it-l10n-Builder-Parker' ), $response['response']['code'], $response['response']['message'] );
		}
		...
		if ( '{' === substr( $body, 0, 1 ) ) {
			...
			if ( is_array( $error ) && isset( $error['error'] ) && is_array( $error['error'] ) && isset( $error['error']['type'] ) && isset( $error['error']['message'] ) ) {
				return sprintf( __( '<p>Unable to get changelog data at this time.</p><p>Error <code>%1$s</code>: %2$s</p>', 'it-l10n-Builder-Parker' ), $error['error']['type'], $error['error']['message'] );
			} else {
				return __( '<p>Unable to get changelog data at this time.</p><p>Error: Unrecognized response from iThemes API server.</p>', 'it-l10n-Builder-Parker' );
			}
		}
