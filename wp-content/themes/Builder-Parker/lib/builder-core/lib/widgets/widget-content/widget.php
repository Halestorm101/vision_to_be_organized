...
if ( ! class_exists( 'ITWidgetContent' ) ) {
	class ITWidgetContent extends WP_Widget {
		function __construct() {
			$widget_ops = array( 'classname' => 'widget-it-content', 'description' => __( 'Add "Widget Content" entries to a sidebar', 'it-l10n-Builder-Parker' ) );
			$control_ops = array( 'width' => 400 );
			
			parent::__construct( 'it_widget_content', __( 'Widget Content', 'it-l10n-Builder-Parker' ), $widget_ops, $control_ops );
		}
...
?>
	<div class="widget-content clearfix">
		...
		<?php if ( 'yes' == builder_get_theme_setting( 'widget_content_edit_link' ) ) : ?>
			<?php edit_post_link( __( 'Edit this entry.', 'it-l10n-Builder-Parker' ), '<p class="edit-entry-link">', '</p>', $post->ID ); ?>
		<?php endif; ?>
	</div>
<?php
...
?>
	<?php if ( ! empty( $entries ) ) : ?>
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'it-l10n-Builder-Parker' ); ?></label>
			<?php $form->add_text_box( 'title', array( 'class' => 'widefat' ) ); ?>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'entry_id' ); ?>"><?php _e( 'Widget Content Entry:', 'it-l10n-Builder-Parker' ); ?></label><br />
