...
//					wp_register_widget_control( $id, $widget['name'], array( $this, 'false_widget_control' ) );
		...
		function add_widget_style_dropdown( $args ) {
			...
			$description = apply_filters( 'builder_filter_widget_style_input_description', __( 'Your Builder child theme offers different widget styles. Select one from the drop down for a different look for this widget.', 'it-l10n-Builder-Parker' ) . '<br />' );
