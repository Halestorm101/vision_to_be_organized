...
if ( ! class_exists( 'ITDuplicateSidebarWidgets' ) ) {
	class ITDuplicateSidebarWidgets extends WP_Widget {
		...
		function __construct() {
			$widget_ops = array( 'classname' => 'it_duplicate_sidebar', 'description' => __( 'Duplicate another sidebar&#8217;s widgets', 'it-l10n-Builder-Parker' ) );
			$control_ops = array( 'width' => 350 );
			
			parent::__construct( 'it_duplicate_sidebar', __( 'Duplicate Sidebar', 'it-l10n-Builder-Parker' ), $widget_ops, $control_ops );
...
?>
	<p>
		<label for="<?php echo $this->get_field_id( 'sidebar' ); ?>"><?php _e( 'Sidebar to Duplicate:', 'it-l10n-Builder-Parker' ); ?></label><br />
...
<?php
		...
		function store_registered_sidebars() {
			...
			foreach ( (array) $wp_registered_sidebars as $sidebar ) {
				$index = ( ! empty( $sidebar['layout'] ) ) ? __( 'Layout Sidebars', 'it-l10n-Builder-Parker' ) : __( 'Non-Layout Sidebars', 'it-l10n-Builder-Parker' );;
