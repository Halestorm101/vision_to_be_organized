		...
		<div class="loop">
			<div class="loop-header">
				<h4 class="loop-title">
					<?php
						...
						if ( is_category() ) { // Category Archive
							$title = sprintf( __( 'Archive for %s', 'it-l10n-Builder-Parker' ), single_cat_title( '', false ) );
						}
						else if ( is_tag() ) { // Tag Archive
							$title = sprintf( __( 'Archive for %s', 'it-l10n-Builder-Parker' ), single_tag_title( '', false ) );
						}
						else if ( is_tax() ) { // Tag Archive
							$title = sprintf( __( 'Archive for %s', 'it-l10n-Builder-Parker' ), builder_get_tax_term_title() );
						}
							...
							$title = sprintf( __( 'Author Archive for %s', 'it-l10n-Builder-Parker' ), get_the_author() );
						}
						else if ( is_year() ) { // Year-Specific Archive
							$title = sprintf( __( 'Archive for %s', 'it-l10n-Builder-Parker' ), get_the_time( 'Y' ) );
						}
						else if ( is_month() ) { // Month-Specific Archive
							$title = sprintf( __( 'Archive for %s', 'it-l10n-Builder-Parker' ), get_the_time( 'F Y' ) );
						}
						else if ( is_day() ) { // Day-Specific Archive
							$title = sprintf( __( 'Archive for %s', 'it-l10n-Builder-Parker' ), get_the_date() );
						}
						else if ( is_time() ) { // Time-Specific Archive
							$title = __( 'Time Archive', 'it-l10n-Builder-Parker' );
						}
						else { // Default catchall just in case
							$title = __( 'Archive', 'it-l10n-Builder-Parker' );
						}
				...
				<?php	if ( is_category() ) { // Category Archive ?>
							<h2 class="category-description"><?php echo category_description(); ?></h2>
				<?php	} ?>
